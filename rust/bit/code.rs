...
fn digit(chr: char, base: u8) -> Result<u8> {
	let out = match chr {
...
fn parse_int(str: &str, base: u8) -> Result<Vec<u32>> {
	let mut out = vec![0];
...
fn mul_add(num: &mut Vec<u32>, by: u8, add: u8) {
	let by = by as u64;
...
fn div_mod(num: &mut Vec<u32>, by: u8) -> u8 {
	let by = by as u64;
	let mut carry = 0;
...
fn is_zero(num: &mut Vec<u32>) -> bool {
